...
Modified.pos <- data.frame(str_locate(peptides$Peptide, "[A-Z.][\\+\\-]\\d+"))$start

# add all this information to peptides table
peptides <- data.frame(peptides, Base.Peptide, Modification, Modified.aa, Modified.pos)

head(peptides)
```

To get the counts of the mass-shifts on the  methionines, for example you can do something like this:
```{r}
testhits<-peptides %>% group_by(Modification) %>%summarize(MetHits=sum(Modified.aa=='M'))
head(testhits)
```

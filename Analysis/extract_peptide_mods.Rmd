...
{
  cat("Reading data from sample", sample, "\n")
  # input filenames
...
{
  cat("Counting modifications between", min_mod, "and", max_mod, "\n")
...
#writeDelimFromList(PSMdata.list, "_PSM_mods.csv.gz", "./GeneratedDataFiles/PSMHits", gz = TRUE)

# next, count all the mods
mod_count.list <- lapply(PSMdata.list, countMods, -2, 2) # limit mod range for now, for development
writeCSVFromList(mod_count.list, "_PSM_mod_counts.csv.gz",
                   "./GeneratedDataFiles/PSMHits", gz = TRUE)
```
